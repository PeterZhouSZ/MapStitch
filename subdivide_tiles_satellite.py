...
if __name__ == "__main__":
	...
	files = list(glob.iglob('coastlines_15\\terrain\\*.png'))

	# size = 128
	# subdiv = 5
	sizes = [128, 64, 32]  # 256#, 64, 32] #[256, 128, 64, 32]
	subdivs = [4, 8, 16]  # 2# ,  8, 16] #[2, 5, 10, 20]
	...
	for i in range(len(sizes)):
		...
		all_land_tiles = np.empty([num_images, size, size, 3])
		all_water_tiles = np.empty([num_images, size, size, 3])
		...
		for filename in tqdm(files):  # glob.glob('coastlines_terrain_14/*.png'): #assuming png format
			...
			filename_satellite = 'coastlines_15\\satellite\\' + filename_without_folder
			...
			m = re.match(r'15_(.*),(.*).png', filename_without_folder)
			lat = float(m.group(1))
			lon = float(m.group(2))

			for bin_tile, sat_tile in zip(subimgs, sat_subimgs):
				if len(set(bin_tile.flatten())) > 1:  # not all zeros or all ones
					continue
				# 	all_equal += 1
				#

				if bin_tile[0][0] == 0:  # water
					all_water_tiles[idx_water] = sat_tile
					idx_water += 1
				else:  # land
					all_land_tiles[idx_land] = sat_tile
					idx_land += 1
		...
		all_land_tiles = all_land_tiles[0:idx_land]
		all_water_tiles = all_water_tiles[0:idx_water]
		#coordinates = coordinates[0:idx]
		num_all_land = all_land_tiles.shape[0]
		num_all_water = all_water_tiles.shape[0]
		...
		hdf5_path = 'data/satellite_zoom_' + str(size) + '_images.hdf5'
		hdf5_file = h5py.File(hdf5_path, mode='w')

		land_shape = (num_all_land, size, size, 3)  # image_data.shape  #
		hdf5_file.create_dataset("land", land_shape, dtype='uint8')
		hdf5_file["land"][...] = all_land_tiles

		water_shape = (num_all_water, size, size, 3)  # image_data.shape  #
		hdf5_file.create_dataset("water", water_shape, dtype='uint8')
		hdf5_file["water"][...] = all_water_tiles
		...
		print('...file contains {} land images and {} water images'.format(hdf5_file["land"].shape[0], hdf5_file["water"].shape[0]))
		hdf5_file.close()
