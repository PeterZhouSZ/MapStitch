...
def generate_map_OLD(tiles_w, tiles_h, image_size, canvas, indices, success, errors, fixed_tile_list, images, identifiers, edge_hash, corner_hash, downsampling_factor=0):
	...
	water_color = (9, 25, 38)
	all_water_tile = water_color * np.ones((image_size, image_size, 3))#np.zeros((image_size, image_size))
	all_land_tile = 255 * np.ones((image_size, image_size))

	all_water_edge = water_color * np.ones((image_size, 3))
	all_water_edge = all_water_edge.flatten('F')

	if downsampling_factor > 0:
		all_water_edge = water_color * np.ones((int(image_size/downsampling_factor), 3))
		all_water_edge = all_water_edge.flatten('F')
	...
	while not np.all(success):
		...
		if x != 0:  # if not left border, consider edge with left neighbor
			...
			if downsampling_factor > 0:
				tile_left_right_edge = tile_left_right_edge.reshape(-1, downsampling_factor).mean(axis=1)
			left_all_water = np.allclose(tile_left_right_edge, all_water_edge, rtol=2)
		...
		if y != 0:  # if not top border, consider edge with top neighbor
			...
			if downsampling_factor > 0:
				tile_top_bottom_edge = tile_top_bottom_edge.reshape(-1, downsampling_factor).mean(axis=1)
			top_all_water = np.allclose(tile_top_bottom_edge, all_water_edge, rtol=2)
		...
		if x == 0:  # handle first column (disregard left neighbor)
			...
			matches_y, orientations = zip(*identifiers[tuple(list_idxs_y)])
			...
			if num_best > 0:
				...
				if top_all_water and random.random() > 0.125:
					#print('top all water!')
					select_tile(all_water_tile, current_index, num_images, 0, canvas, indices, success)
				elif top_all_land and random.random() > 0.125:
					select_tile(all_land_tile, current_index, num_images+1, 0, canvas, indices, success)
				else:
			...
			matches_x, orientations = zip(*identifiers[tuple(list_idxs_x)])
			...
			if len(list_idxs) > 0:
				matches, orientations = zip(*identifiers[tuple(list_idxs)])
...
def generate_large_map(tiles_w, tiles_h, image_size, canvas, canvas_large, indices, success, errors, fixed_tile_list, images, images_large, identifiers, edge_hash, corner_hash):
	num_tiles = tiles_w * tiles_h
	print('{} tiles'.format(num_tiles))
	num_images = images.shape[0]

	image_large_size = images_large.shape[1]
	print(image_large_size)
	channels = 1
	if len(images.shape) > 3:
		channels = images.shape[3]
		canvas = np.zeros((tiles_h * tiles_w, image_size, image_size, channels), dtype='uint8')
		canvas_large = np.zeros((tiles_h * tiles_w, image_large_size, image_large_size, channels), dtype='uint8')
	else:
		canvas = np.zeros((tiles_h * tiles_w, image_size, image_size), dtype='uint8')
		canvas_large = np.zeros((tiles_h * tiles_w, image_large_size, image_large_size), dtype='uint8')
	...
	water_color = (9, 25, 38)
	all_water_tile = water_color * np.ones((image_size, image_size, 3))#np.zeros((image_size, image_size))
	all_water_large_tile = water_color * np.ones((image_large_size, image_large_size, 3))#np.zeros((image_size, image_size))
	all_land_tile = 255 * np.ones((image_size, image_size))

	all_water_edge = water_color * np.ones((image_size, 3))
	all_water_large_edge = water_color * np.ones((image_large_size, 3))
	all_water_edge = all_water_edge.flatten('F')
	all_water_large_edge = all_water_large_edge.flatten('F')
	...
	while not np.all(success):
		...
		if current_index == 0: # for start tile, randomly initialize tile
			...
			while not good_pick:
				...
				indices[0] = random_index
				canvas[0, :, :] = images[random_index]
				canvas_large[0, :, :] = images_large[random_index]
				good_pick = True
		...
		if x == 0:  # handle first column (disregard left neighbor)
			...
			if num_best > 0:
				...
				if top_all_water and random.random() > 0.125:
					#print('top all water!')
					select_tile(all_water_tile, current_index, num_images, 0, canvas, indices, success)
					select_tile(all_water_large_tile, current_index, num_images, 0, canvas_large, indices, success)
				else:
					# write selected tile to canvas
					select_tile(images[index], current_index, index, orientation, canvas, indices, success)
					select_tile(images_large[index], current_index, num_images, orientation, canvas_large, indices, success)
					errors[x][y] += distance
				continue
		elif y == 0:  # handle first row (disregard top neighbor)
		...
		# do stochastic backtracking
		failure = backtrack(failure, current_index, success, indices, tiles_w)
	return canvas, canvas_large, indices, errors
...
def generate_map(tiles_w, tiles_h, image_size, canvas, indices, success, errors, fixed_tile_list, images, identifiers, edge_hash, corner_hash):
	...
	while not np.all(success):
		...
		if y != 0:  # if not top border, consider edge with top neighbor
			tile_top_bottom_edge = get_tile_edge_from_canvas(canvas, image_size, BOTTOM, current_index - tiles_w,
															 flip=False)
			top_all_water = np.allclose(tile_top_bottom_edge, all_water_edge, rtol=2)
			top_all_land = np.array_equal(tile_top_bottom_edge, 255 * np.ones(image_size))

		if x == 0:  # handle first column (disregard left neighbor)
			list_idxs_y, distances_y = get_nearest(edge_hash, tile_top_bottom_edge, num_results=max_results,
			...
			list_idxs_x, distances_x = get_nearest(edge_hash, tile_left_right_edge, num_results=max_results,
												   max_distance=max_distance)
			matches_x, orientations = zip(*identifiers[tuple(list_idxs_x)])
			# print(distances_x.shape)
			best_matches_x = []
...
def generate_map_all_edges(tiles_w, tiles_h, image_size, canvas, indices, success, errors, fixed_tile_list, images, identifiers, edge_hash, corner_hash, opposite_edges_hash, three_edges_hash, four_edges_hash):
	...
	while not np.all(success):
		...
		if x != 0:  # if not left border, consider edge with left neighbor
			...
			tile_left_right_edge = get_tile_edge_from_canvas(canvas, image_size, RIGHT, current_index - 1, flip=True)
			left_all_water = np.array_equal(tile_left_right_edge, all_water_edge)
			left_all_land = np.array_equal(tile_left_right_edge, 255*np.ones(image_size))

		if y != 0:  # if not top border, consider edge with top neighbor
			constrained_top = True
			tile_top_bottom_edge = get_tile_edge_from_canvas(canvas, image_size, BOTTOM, current_index - tiles_w, flip=False)
			top_all_water = np.array_equal(tile_top_bottom_edge, all_water_edge)
			top_all_land = np.array_equal(tile_top_bottom_edge, 255*np.ones(image_size))
		...
		if x != tiles_w-1 and fixed_tile_list[y * tiles_w + x + 1] == 1:  # if not right border, consider edge with right neighbor
			...
			right_all_water = np.array_equal(tile_right_left_edge, all_water_edge)
		...
		if y != tiles_h-1 and fixed_tile_list[(y + 1) * tiles_w + x] == 1:  # if not bottom border, consider edge with bottom neighbor
			...
			bottom_all_water = np.array_equal(tile_bottom_top_edge, all_water_edge)
